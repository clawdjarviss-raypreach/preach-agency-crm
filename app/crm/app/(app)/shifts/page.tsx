  ...
  return (
    <div className="app-shell flex">
      ...
      <main className="flex-1 p-6">
        ...
        <p className="page-subtitle">V0: shifts are tied to your dev-login role via seeded demo users.</p>
        ...
        <div className="mt-3 grid gap-3 sm:grid-cols-4">
          <div className="card">
            <div className="card-label">Current Pay Period</div>
            <div className="card-value">{periodHours} h</div>
            <div className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">{periodLabel}</div>
          </div>
          <div className="card">
            <div className="card-label">Period Shifts</div>
            <div className="card-value">{periodShifts.length}</div>
          </div>
          <div className="card">
            <div className="card-label">Last 7 days</div>
            <div className="card-value">{recentHours} h</div>
          </div>
          <div className="card">
            <div className="card-label">Open shift</div>
            <div className="card-value">{openShift ? 'Running' : 'None'}</div>
          </div>
        </div>
        ...
        {openShift && (
          <div className="mt-4 card text-sm bg-zinc-50 dark:bg-zinc-950">
            ...
            <form action="/api/shifts/clock-out" method="post" className="mt-3 grid gap-2 sm:grid-cols-3">
              ...
              <div className="sm:col-span-3">
                <button className="btn-outline">Clock out</button>
              </div>
            </form>
          </div>
        )}
        ...
        {!openShift && (
          <div className="mt-4 card text-sm text-zinc-600 dark:text-zinc-400">
            No open shift right now. Clock in to start one.
          </div>
        )}

        <div className="mt-6 table-wrap overflow-auto">
                      ...
                      {reportState === 'yes' && s.report && (
                        <details className="group">
                          <summary className="cursor-pointer select-none">
                            ...
                            <span className="text-xs brand-link group-open:hidden">View</span>
                            <span className="text-xs brand-link hidden group-open:inline">Hide</span>
                          </summary>
                          <div className="mt-2">
                            <ShiftReportView
                              report={{
